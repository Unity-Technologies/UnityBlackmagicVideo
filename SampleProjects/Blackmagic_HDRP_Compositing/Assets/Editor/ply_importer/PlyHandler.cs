...
namespace ThreeDeeBear.Models.Ply
{
    ...
    public static class PlyHandler
    {
        ...
        static PlyResult ParseAscii(List<string> plyFile, PlyHeader header)
        {
            ...
            plyFile.GetRange(faceStartIndex, header.FaceCount).ForEach(face => { triangles.AddRange(GetTriangles(face, header)); });
            return new PlyResult(vertices, triangles, colors);
        }

        static Vector3 ParseVertex(string[] xyzrgb, PlyHeader header)
        {
            decimal dx, dy, dz;
        ...
        static Color ParseColor(string[] xyzrgb, PlyHeader header)
        {
            int r = 255;
            int g = 255;
            int b = 255;
        ...
        static List<int> GetTriangles(string faceVertexList, PlyHeader header)
        {
        ...
        static PlyResult ParseBinaryLittleEndian(string path, PlyHeader header)
        {
        ...
        static List<Vector3> GetVertices(byte[] bytes, PlyHeader header, out List<Color> colors)
        {
            var vertices = new List<Vector3>();
        ...
        static List<int> GetTriangles(byte[] bytes, PlyHeader header)
        {
        ...
        static int GetByteCountPerVertex(PlyHeader header)
        {
            int bpvc = 4; // bytes per vertex component
            int bpcc = 1; // bytes per color component
            // todo: support other types than just float for vertex components and byte for color components
        ...
        static List<int> QuadToTriangles(List<int> quad)
        {
