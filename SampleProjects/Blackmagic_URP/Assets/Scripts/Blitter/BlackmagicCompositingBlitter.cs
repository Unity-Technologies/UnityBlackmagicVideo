...
namespace Unity.Media.Blackmagic
{
    ...
    public sealed class BlackmagicCompositingBlitter : MonoBehaviour
    {
        class ShaderIDs
        {
            public const string _CompositingShaderName = "Hidden/BlackmagicVideo/Shader/SimpleCompositing";
            public const string _FinalBlitShaderName = "Hidden/BlackmagicVideo/Shader/SimpleBlit";
        ...
        void InitializeMeshOutput()
        {
            ...
            var shaderBlit = Shader.Find(ShaderIDs._FinalBlitShaderName);
            if (shaderBlit != null)
            {
                m_SimpleBlitMaterial = new Material(shaderBlit);
            }
            else
            {
                Debug.LogError(String.Format("Shader not found: {0}", ShaderIDs._FinalBlitShaderName));
            }

            var shaderCompositing = Shader.Find(ShaderIDs._CompositingShaderName);
            if (shaderCompositing != null)
            {
                m_CompositingMaterial = new Material(shaderCompositing);
            }
            else
            {
                Debug.LogError(String.Format("Shader not found: {0}", ShaderIDs._CompositingShaderName));
            }
        ...
        void OnDisable()
        {
            ...
            if (m_Mesh != null)
            {
                ...
                DestroyImmediate(m_SimpleBlitMaterial);
                m_Mesh = null;
                m_SimpleBlitMaterial = null;
            }
        }
    }
}
