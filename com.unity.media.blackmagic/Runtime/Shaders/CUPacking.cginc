...
float2 getSamplingUV(float2 uv, float4 ts)
{
    ...
    uv.y = clamp((floor((uv.y * ts.w) / 2 + 0) * 2 + 0.5) * ts.y, 0, 1);
#elif defined(BLACKMAGIC_DEINTERLACE_EVEN)
    uv.y = clamp((floor((uv.y * ts.w) / 2 + 1) * 2 - 0.5) * ts.y, 0, 1);
#endif

    // adjustment for picking needs to be quite small to ensure /2 width textures are unaffected.
    uv.x += .5 * ts.x;
    //uv.y += .5 * ts.y;
...
float3 ClipCbCrSignal(float3 signal)
{
    // Y'CbCr -> Y'Cb'Cr'
    signal.g = signal.g * (219.0 / 255.0) + (16.0 / 255.0);
    signal.b = signal.b * (219.0 / 255.0) + (16.0 / 255.0);
